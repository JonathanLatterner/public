...
class ChatPage extends React.Component {
  ...
  filterPeople = (list) => {
    _logger(list);
    _logger("correspondents", this.state.correspondents);
    let newList = list.filter((item) => {
      return !this.state.correspondents.some(
  ...
  onAllPeopleSuccess = (response) => {
    this.setState(() => {
      return {
        people: this.filterPeople(response.item.pagedItems),
      };
    });
    _logger(response);
  };
  ...
  onSendSuccess = (response) => {
    this.setState((prevState) => {
      ...
      message.senderId = this.props.currentUser.id;
      let date = new Date();
      message.dateSent = date.getTime();
      list.push(message);
