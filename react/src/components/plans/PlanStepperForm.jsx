...
class PlanStepperForm extends React.Component {
  ...
  checkErrors = (props) => {
    ...
    for (const value in props.values) {
      if (props.values[value].length === 0) {
        arr.push(`${value} is a required Field`);
      }
    }
    if (arr.length === 0) {
      arr.push(
        "Plan Complete, click Submit or reset the form to start all over"
      );
    }
    return arr;
  };
                ...
                <Grid item>
                  <Button
                    ...
                    onClick={() => {
                      formikProps.values.agendas
                        ? arrayHelpers.insert(
                            formikProps.values.agendas.length,
                            ""
                          )
            ...
            <Form>
              ...
              {this.state.activeStep === this.getSteps().length && (
                <Grid container spacing={3} alignItems="center">
                  <Grid item xs={1}></Grid>
                  <Grid item xs={11}>
                    {this.checkErrors(formikProps).map((error, index) => (
                      <Typography color="error" key={index + error}>
                        {error}
                      </Typography>
                    ))}
                  </Grid>
